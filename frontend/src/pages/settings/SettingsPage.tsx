...
import {
  ...
  Table, Menu, SegmentedControl, useMantineColorScheme,
} from '@mantine/core'
  ...
  return (
    <div className="settings-page">
      ...
      <div className="settings-header">
        ...
        {activeTab !== 'users' && activeTab !== 'system' && activeTab !== 'products' && activeTab !== 'theme' && (
...
function ThemeTab() {
  ...
  return (
    <Card shadow="xs" padding="lg" radius="md" withBorder>
      <Group gap={8} mb="md">
        <IconPalette size={20} color="var(--app-primary)" />
        <Text fw={700} size="lg">ธีม</Text>
      </Group>
      <Stack gap="lg">
        <div>
          <Text size="sm" fw={600} mb="xs">เลือกโหมดการแสดงผล</Text>
          <Text size="xs" c="dimmed" mb="md">
            เลือกธีมที่ต้องการใช้งาน หรือให้ระบบปรับตามการตั้งค่าของอุปกรณ์อัตโนมัติ
          </Text>
          <SegmentedControl
            value={colorScheme}
        ...
        <div className="settings-info-box">
          <Text size="sm" fw={600} mb={4}>เกี่ยวกับธีม</Text>
          <Text size="xs" c="dimmed">
            <strong>สว่าง (Light):</strong> ธีมสว่างเหมาะสำหรับการใช้งานในที่สว่าง<br/>
            <strong>มืด (Dark):</strong> ธีมมืดช่วยลดแสงจ้าเมื่อใช้งานในที่มืด ถนอมสายตา<br/>
            <strong>อัตโนมัติ (Auto):</strong> ปรับตามการตั้งค่าของอุปกรณ์อัตโนมัติ
          </Text>
        </div>
      </Stack>
    </Card>
...
function ProductAttributesTab() {
  ...
  const addValueMutation = useMutation({
    ...
    onSuccess: (_: any, vars: { groupId: number; value: string }) => {
    ...
    <Stack gap="lg">
      ...
      <Card shadow="xs" padding="lg" radius="md" withBorder>
        ...
        <Text size="sm" c="dimmed" mb="md">
          กำหนดกลุ่มคุณสมบัติสินค้าเพื่อใช้กรองและจัดหมวดหมู่: เช่น หมวดสินค้า → แบรนด์ → รายการสินค้า
        </Text>
        <Group>
      ...
      {groups.length === 0 ? (
        ...
        groups.map((g: any, gi: number) => {
          ...
          const levelLabel = gi === 0 ? 'ย่อยหลัก' : `ย่อย ${gi}`
          return (
            <Card key={g.id} shadow="xs" padding="lg" radius="md" withBorder
              ...
              {/* Values List — numbered sub-items */}
              {values.length > 0 && (
                <div style={{ marginBottom: 12 }}>
                  {values.map((v: any, vi: number) => (
                    <div key={v.id} style={{
                      ...
                      background: vi % 2 === 0 ? 'rgba(0,0,0,0.02)' : 'transparent',
                      ...
                      {editingValue === v.id ? (
                        <Group gap={4} style={{ flex: 1 }}>
                          ...
                          <ActionIcon size="sm" variant="light" color="gray" onClick={() => setEditingValue(null)}>
                            <IconX size={14} />
                          </ActionIcon>
                        </Group>
                      ) : (
                        <>
                          <Text size="sm" fw={500} style={{ flex: 1 }}>{v.value}</Text>
                          <Group gap={4}>
                            <Tooltip label="แก้ไข">
                              <ActionIcon size="sm" variant="light" color={color}
                                onClick={() => { setEditingValue(v.id); setEditValueName(v.value) }}>
                                <IconEdit size={14} />
                              </ActionIcon>
                            </Tooltip>
                            <Tooltip label="ลบ">
                              <ActionIcon size="sm" variant="light" color="red"
                                onClick={() => setDeleteValueModal(v)}>
                                <IconTrash size={14} />
                              </ActionIcon>
                            </Tooltip>
                          </Group>
