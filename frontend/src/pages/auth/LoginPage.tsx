...
import { TextInput, PasswordInput, Button, Checkbox, Modal, Badge } from '@mantine/core'
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (companies.length <= 1) {
        // Only one company — go straight in
        setAuth(token, user, companies, activeCompany)
        notifications.show({ title: 'เข้าสู่ระบบสำเร็จ', message: `ยินดีต้อนรับ ${user.fullName || user.full_name}`, color: 'green' })
        navigate('/')
      } else {
        // Multiple companies — ask which one
        setPendingData({ token, user, companies, activeCompany })
        setSelectedCompanyId(activeCompany?.company_id || companies[0]?.company_id || null)
  ...
  const handleSelectCompany = async () => {
    if (!pendingData || !selectedCompanyId) return
    setSwitchLoading(true)
    try {
      // First set auth so we have a token for the switch-company request
      setAuth(pendingData.token, pendingData.user, pendingData.companies, pendingData.activeCompany)

      // Call switch-company to get a token scoped to the chosen company
      const res = await api.post(
        '/auth/switch-company',
        { companyId: selectedCompanyId },
        { headers: { Authorization: `Bearer ${pendingData.token}` } }
      )
      const { token: newToken, companies, activeCompany } = res.data
      switchCompany(activeCompany, newToken, companies)

      notifications.show({
        title: 'เข้าสู่ระบบสำเร็จ',
        message: `ยินดีต้อนรับ ${pendingData.user.fullName || pendingData.user.full_name} — ${activeCompany.company_name}`,
        color: 'green',
      })
      navigate('/')
    } catch (err: any) {
  ...
  if (selectingCompany && pendingData) {
    const userName = pendingData.user.fullName || pendingData.user.full_name || ''
      ...
      <div className="login-page">
        ...
        <div className="cs-card">
          ...
          <div className="cs-header">
            ...
            <div className="cs-brand-row">
              ...
              <span className="cs-brand-label">Bookdee POS</span>
            </div>
            <div className="cs-user-block">
              <div className="cs-user-avatar">{initials}</div>
              <div>
                <div className="cs-user-greeting">ยินดีต้อนรับ 👋</div>
                <div className="cs-user-name">{userName}</div>
              </div>
            </div>
            <p className="cs-header-hint">กรุณาเลือกกิจการที่ต้องการเข้าใช้งาน</p>
          </div>

          {/* Company List */}
          <div className="cs-body">
            <div className="cs-company-count">
              {pendingData.companies.length} กิจการที่คุณเข้าถึงได้
            </div>

            <div className="cs-list">
              {pendingData.companies.map((c, i) => {
                const isActive = selectedCompanyId === c.company_id
                return (
                  <button
                    key={c.company_id}
                    type="button"
                  ...
                  >
                    {/* Number badge */}
                    <div className={`cs-item-num${isActive ? ' cs-item-num--active' : ''}`}>
                      {isActive ? <IconCheck size={14} /> : i + 1}
                    </div>

                    {/* Icon */}
                    <div className={`cs-item-icon${isActive ? ' cs-item-icon--active' : ''}`}>
                      {c.logo_url ? (
                        <img src={c.logo_url} alt={c.company_name} className="company-logo-img" />
                      ) : (
                    ...
                    <div className="cs-item-info">
                      <span className="cs-item-name">{c.company_name}</span>
                      <div className="cs-item-meta">
                        <span className={`cs-role-dot cs-role-dot--${c.role}`} />
                        <span className="cs-item-role">{roleLabel[c.role] || c.role}</span>
                        {c.is_default && (
                          <span className="cs-default-pill">ค่าเริ่มต้น</span>
                        )}
                      </div>
                    </div>
                    ...
                    <IconChevronRight
                      size={16}
                      className={`cs-item-arrow${isActive ? ' cs-item-arrow--active' : ''}`}
                    />
                  </button>
                )
              })}
            ...
            <div className="cs-actions">
              ...
              <button
                type="button"
                className={`cs-btn-enter${!selectedCompanyId ? ' cs-btn-enter--disabled' : ''}`}
                disabled={!selectedCompanyId || switchLoading}
                onClick={handleSelectCompany}
              >
                {switchLoading ? (
                  <span className="cs-spinner" />
                ) : (
                  <>เข้าสู่กิจการ <IconArrowRight size={16} /></>
                )}
              </button>
            </div>

            <div className="cs-footer">Powered by <strong>Bookdee POS</strong> v1.0</div>
          </div>
        </div>
        ...
        >
          <div className="login-error-modal-body">
            <ErrorIcon />
            <h3 className="login-error-title">เกิดข้อผิดพลาด</h3>
