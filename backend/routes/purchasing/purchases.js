...
const { createPOSchema } = require('../../middleware/schemas')

router.use(auth, companyGuard)

// Helper: get VAT rate from company settings
async function getVatRate(companyId) {
  ...
  if (rows.length > 0 && rows[0].settings) {
    ...
    if (settings.vat_enabled === false) return 0
    return (settings.vat_rate || 7) / 100
  }
...
router.post('/', roleCheck('owner', 'admin', 'manager'), validate(createPOSchema), async (req, res) => {
  ...
  try {
    ...
    // Calculate totals
    const vatRate = await getVatRate(req.user.companyId)
...
router.post('/receipts', roleCheck('owner', 'admin', 'manager'), async (req, res) => {
  ...
  try {
    ...
    if (req.body.taxInvoiceNumber || req.body.createInvoice !== false) {
      ...
      const invVatRate = await getVatRate(req.user.companyId)
      const invVat = invSubtotal * invVatRate
    ...
    // === Auto Journal: รับสินค้าเข้าสต๊อก ===
    const grnCostTotal = items.reduce((s, i) => s + ((i.receivedQuantity || 0) * (i.costPerUnit || 0)), 0)
    if (grnCostTotal > 0) {
      const grnJournalLines = [
        // Debit: Inventory (1200) — สินค้าเพิ่ม
        { accountCode: '1200', debit: grnCostTotal, credit: 0, description: `รับสินค้า ${grnNumber}` },
        // Credit: Accounts Payable (2100) — เจ้าหนี้การค้าเพิ่ม
        { accountCode: '2100', debit: 0, credit: grnCostTotal, description: `เจ้าหนี้จาก ${grnNumber}` },
      ]

      await createJournalEntry(connection, {
        companyId: req.user.companyId,
        entryDate: receivedDate || new Date().toISOString().slice(0, 10),
        description: `รับสินค้า ${grnNumber} (PO: ${pos[0].po_number})`,
        referenceType: 'GRN', referenceId: grnResult.insertId,
        createdBy: req.user.id, lines: grnJournalLines,
      })
    }
...
router.post('/payments', roleCheck('owner', 'admin', 'manager', 'accountant'), async (req, res) => {
  ...
  try {
    ...
    // === Auto Journal: จ่ายเงินซื้อ ===
    const payAmount = parseFloat(amount)
    if (payAmount > 0) {
      const payJournalLines = [
        // Debit: Accounts Payable (2100) — ลดเจ้าหนี้
        { accountCode: '2100', debit: payAmount, credit: 0, description: `ชำระเงิน ${paymentNumber}` },
        // Credit: Cash/Bank (1100) — เงินออก
        { accountCode: '1100', debit: 0, credit: payAmount, description: `จ่ายค่าสินค้า ${paymentNumber}` },
      ]

      await createJournalEntry(connection, {
        companyId: req.user.companyId,
        entryDate: paymentDate || new Date().toISOString().slice(0, 10),
        description: `ชำระเงินค่าสินค้า ${paymentNumber} (Invoice: ${invoice.invoice_number})`,
        referenceType: 'PURCHASE_PAYMENT', referenceId: invoiceId,
        createdBy: req.user.id, lines: payJournalLines,
      })
    }
