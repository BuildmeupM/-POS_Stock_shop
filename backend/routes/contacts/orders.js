...
router.put('/:id/status', async (req, res) => {
  ...
  try {
    await connection.beginTransaction()
    const { orderStatus, trackingNumber, shippingProvider, paymentMethod, paymentChannelId, paymentReference } = req.body
    ...
    // ชำระเงินแล้ว → อัพเดต payment_status + วิธีชำระ
    if (orderStatus === 'confirmed') {
      updates += ", payment_status = 'confirmed'"
      if (paymentMethod) { updates += ', payment_method = ?'; params.push(paymentMethod) }
      if (paymentReference) { updates += ', payment_proof_url = ?'; params.push(paymentReference) }
    }
